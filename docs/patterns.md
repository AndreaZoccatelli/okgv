...
**Shape.** Question-answer (or instruction-response) pairs over a subject hierarchy, each graded `easy`/`medium`/`hard`, balanced across both subject and difficulty. This is the archetype the README opens with (cats/dogs × easy/medium/hard generalized).
